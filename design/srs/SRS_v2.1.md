...
**Version 2.1**  ·  **7 August 2026**  ·  **Status: for implementation**
...
> **Note on band thresholds.** Rescaling stretches the endpoints of a
> distribution; it does not change its shape. Because the product is
> right-skewed, a large share of divisions will still fall in the lower bands
> even after step 2. The default thresholds in §2.7 are therefore a placeholder.
> They must be re-derived from the observed distribution once real values are
> loaded — see §2.7 and FR-4.13.
...
|---|---|---|
| Variable normalisation (§2.2) | Each indicator, to put units on a common scale | **National** — all 330 divisions **[P-2]** |
| Index renormalisation (step 2 above) | The vulnerability index, to fill the colour scale | **Provincial** **[P-14]** |

Variable normalisation stays national so that the score composition panel
(FR-5.9) shows normalised values a reader can compare between provinces, and so
the raw index is built on one measurement scale even though the published score
is rescaled per province.
...
**These thresholds are a placeholder.** No indicator values have been loaded
yet, so the distribution of the published index is not known. Even thresholds
are appropriate only if scores spread evenly, and a renormalised product will
not: rescaling moves the endpoints of a distribution without changing its shape,
so the right-skew of the product survives step 2.

The thresholds must therefore be re-derived once real values exist, as the first
task after the first province is loaded. Because they are configuration
(FR-4.13), no code changes.

Two derivation methods are available and the choice belongs to the expert panel:

- **Quantile** — each band holds an equal share of divisions. Guarantees a
  readable map; the bands are positions, not severities.
- **Natural breaks** — cut points follow groupings in the observed data.
  Reflects real structure; thresholds move if re-derived later, so they should
  be fixed after one review rather than recomputed continuously.
...
| FR-4.13 | The system shall classify scores into five bands with configurable half-open thresholds. The default 0.2 / 0.4 / 0.6 / 0.8 is a placeholder to be re-derived from the observed distribution once real values are loaded (§2.7). Thresholds shall be configuration, changeable without a code change or a migration. |
| FR-4.15 | The system shall provide a report of the observed distribution of the published index — counts per band and the quantile cut points — so thresholds can be set from evidence rather than assumed. |
...
| FR-5.24 | Because the published index is rescaled within each province (§2.1), the map shall state that scores are **relative to the province** and shall not present a single national legend implying one scale. Where divisions from more than one province are shown together, the interface shall say so. |
...
5. Provincial renormalisation of `V_raw`, storing the bounds used.
6. Banding, with thresholds read from configuration.
...
| O-10 | Band thresholds are a placeholder until real values are loaded (§2.7) | The map is unreadable if thresholds are left at even intervals against a skewed distribution |
...
| Data collection and import | 6.2 | FR-2.1 – FR-2.16 | 16 |
| Weighting | 6.3 | FR-3.1 – FR-3.14 | 14 |
| Normalisation and computation | 6.4 | FR-4.1 – FR-4.15 | 19 |
| Map and visualisation | 6.5 | FR-5.1 – FR-5.26 | 26 |
